...
export type Route =
  | { name: 'home' }
  | { name: 'assurance' }
  | { name: 'tool'; toolId: string; projectId?: string; state?: string; methods?: boolean }
...
export function parseRoute(hash: string): Route {
  ...
  if (path === '/assurance' || path === '/assurance/') return { name: 'assurance' };
  ...
  if (s) r.state = s;
  if (params.get('methods') === '1') r.methods = true;
  return r;
}

export function toHash(route: Route): string {
  if (route.name === 'home') return '#/';
  if (route.name === 'assurance') return '#/assurance';
  if (route.name === 'notfound') return route.hash;
  let h = `#/t/${route.toolId}`;
  if (route.projectId) h += `/p/${route.projectId}`;
  const params = new URLSearchParams();
  if (route.state) params.set('s', route.state);
  if (route.methods) params.set('methods', '1');
  if (params.size) h += `?${params.toString()}`;
  return h;
}
